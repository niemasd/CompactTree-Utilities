...
int main(int argc, char** argv) {
    ...
    for(int i = 1; i < argc; ++i) {
        ...
        // positional argument
            ...
            pos_args.emplace_back(curr_arg);
        }
    }
    fn = pos_args[0];
    ...
    if(pos_args.size() == 2) {
        std::ifstream infile(pos_args[1]); std::istringstream ss; std::string first; std::string second;
    ...
    std::istream* infile; CT_NODE_T num_nodes; CT_NODE_T curr;
    std::unordered_map<std::string, std::string>::iterator it; std::unordered_map<std::string, std::string>::iterator it_end = label_map.end();
    if(strcmp(fn, "-") == 0) { infile = &(std::cin); } else { infile = new std::ifstream(fn); }
    while(getline(*infile, line)) {
        compact_tree tree(line, false, true, true); num_nodes = tree.get_num_nodes();
        for(curr = ROOT_NODE; curr < num_nodes; ++curr) {
            if(tree.is_leaf(curr) || include_internal) {
                it = label_map.find(tree.get_label(curr));
                if(it != it_end) { tree.set_label(curr, it->second); }
            }
        }
        tree.print_newick(std::cout); std::cout << std::endl;
    }
